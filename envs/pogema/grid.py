...
from pogema.generator import generate_obstacles, generate_positions_and_targets_fast, \
    get_components
from .grid_config import GridConfig, EnvMode
...
class Grid:
    ...
    def __init__(self, grid_config: GridConfig, add_artificial_border: bool = True, num_retries=10):
        ...
        self.global_obstacles = self.get_obstacles(ignore_borders=True).astype(int)
        self.observed_masks = [np.zeros((self.config.size, self.config.size)).astype(int) for agent_id in range(self.config.num_agents)]
        self.comm_masks = [np.zeros((self.config.size, self.config.size)).astype(int) for agent_id in range(self.config.num_agents)]
        self.globally_observed_mask = np.zeros((self.config.size, self.config.size)).astype(int)
    ...
    def make_mask(self, agent_id):
        x, y = self.get_agents_xy(ignore_borders=True)[agent_id]
        s = self.config.size
        r = self.config.obs_radius
        mask = np.zeros((s, s))
        visible = mask[max(x-r, 0):min(x+r+1, s), max(y-r, 0):min(y+r+1, s)]
        mask[max(x-r, 0):min(x+r+1, s), max(y-r, 0):min(y+r+1, s)] = np.ones(visible.shape)
        self.observed_masks[agent_id] = self.observed_masks[agent_id] | mask.astype(int)
        self.globally_observed_mask = self.globally_observed_mask | mask.astype(int)
        return self.observed_masks[agent_id], mask.astype(int)

    def get_obstacles_for_agent(self, agent_id):
        return self.global_obstacles

    def get_positions(self, agent_id):
        x, y = self.get_agents_xy(ignore_borders=True)[agent_id]
        r = self.config.obs_radius
        other_agents = -self.positions[r:-r, r:-r].astype(int) # use negative to make other agents -1
        other_agents[x,y] = 1
        return other_agents
    ...
    def get_square_target(self, agent_id):
        target = np.zeros((self.config.size, self.config.size))
        target[tuple(self.get_targets_xy(ignore_borders=True)[agent_id])] = 1
        if self.config.env_mode == EnvMode.DECENTRALIZED_COMM:
            for i in range(self.config.num_agents):
                if i != agent_id:
                    target[tuple(self.get_targets_xy(ignore_borders=True)[i])] = -1
