...
class EnvMode(Enum):
    CENTRALIZED_CONCATENATED_CRITIC = 1
    CENTRALIZED_FULL_GLOBAL_CRITIC = 2
    DECENTRALIZED_NO_COMM = 3
    DECENTRALIZED_COMM = 4

class RewardScheme(Enum):
    INDEPENDENT = 1 # +1 any time you reach your target, not shared to other agents
    SHARED_ON_TARGET = 2 # +1 to all agents any time anyone reaches their target
    SHARED_ALL_REACH_TARGET = 3 # +1 to all agents only when everyone reaches their target
    INDEPENDENT_AND_COOPERATIVE_SHARED = 4 # +0.2 any time you reach your target, +1 to all agents when everyone reaches their target


class GridConfig(CommonSettings, ):
    env_mode: EnvMode = EnvMode.CENTRALIZED_CONCATENATED_CRITIC
    reward_scheme: RewardScheme = RewardScheme.INDEPENDENT
