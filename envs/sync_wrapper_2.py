...
# from sync_or_sink.forage_main.cool_forage_env import PettingZooForageEnv
import torch
from torch import nn
from sync_or_sink.forage import utils
import argparse
import numpy as np
import networkx as nx
from sync_or_sink.forage.env_wrapper import ForageEnvironment

from termcolor import cprint

class TiecommPettingZoo(ForageEnvironment):
    """
    ...
    def get_state_size(self):
        ...
        state_size = obs_size * len(self.agents)
        return state_size
    ...
    def reformat_rewards(self, rewards_dict):
        """
        Reformats rewards from a dictionary to a NumPy array of rewards for each agent.
        
        Args:
            rewards_dict (dict): A dictionary containing rewards for each agent.
            
        Returns:
            np.ndarray: A NumPy array of rewards for each agent.
        """
        rewards_list = []
        for agent in sorted(rewards_dict.keys()):
            rewards_list.append(rewards_dict[agent])
        
        return np.array(rewards_list)
    

    def info_adapter(self, terminations: dict, truncations: dict, info: dict):
        """
        combines forage wrappers input (terminated, info) into a dictionary
        called env_info which the policy understands. We aren't doing anything
        with info for now
        """

        env_info = {}
        combined = defaultdict(bool)
        for agent in list(terminations.keys()):
            combined[agent] = terminations[agent] or truncations[agent]
        
        if any(combined.values()):
            env_info['TimeLimit.truncated'] = True
        
        completed_agents = [int(value) for value in terminations.values()]
        env_info['completed_agent'] = completed_agents

        return env_info

    def done_adapter(self, terminations: dict, truncations: dict):
        """
        Turns inputs (terminations, truncations) outputted from cool_forage_env 
        into a boolean done

        ASSUMES THAT TERMINATIONS AND TRUNCATIONS HAVE THE SAME AGENTS
        """
        done = False
